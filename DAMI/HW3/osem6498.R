...
getEntropy <- function(class.freq){
  ...
  # *** ATTENTION !!! *** #
  sum <- 0
  for(i in 1:nrow(class.freq)){
    sum <- sum - (p[i,]*log2(p[i,]))
  }
  
  entropy <- sum #"FIX ME" DONE
...
getInfoGain <- function(split_var, class_var){
  ...
  for(i in 1:nrow(attr.class.freq)){
    ...
    # *** ATTENTION !!! *** #
    child.node.freq <- as.matrix(attr.class.freq[i,]) #"FIX ME" DONE
    ...
    # *** ATTENTION !!! *** #
    entropy_split <- entropy_split + (child.node.entropy*(num_obs_child.node / num_obs_parent.node)) #"FIX ME" DONE
  ...
  for(i in 1:number_of_columns){
    ...
    if(class(data.X[,i]) == "numeric"){
      ...
      for(j in 1:(length(x_sort_unique)-1)){
        ...
        # *** ATTENTION !!! *** #
        cutoff_point <- (x_sort_unique[j]+x_sort_unique[j+1])/2 #"FIX ME" DONE
        # print(cutoff_point) #####
        ...
        # *** ATTENTION !!! *** #
        info_gain_for_this_split <- getInfoGain(lessthan_cutoff, data.Y) #"FIX ME" DONE
        # print(info_gain_for_this_split) ####
...
my.perceptron <- function(data.X, data.Y, max.iter = 500){
  ...
  while(TRUE){
    ...
    # *** ATTENTION !!! *** #
    hypothesis <- w %*% t(as.matrix(X.new)) #"FIX ME" DONE
    ...
    if(all(label.new == data.Y)){ 
      ...
      # *** ATTENTION !!! *** #
      w <- (w + (as.matrix(X.new[rand.sel.index,]) * data.Y[rand.sel.index])) #"FIX ME" DONE
      # print(w)
...
for(i in 100:model$iterations){
  ...
  if(i == model$iterations)
    abline(intercept.i, slope.i,lwd = 3, col="red")
}
...
# *** ATTENTION !!! *** #
k.fold.data.idx <- createFolds(heart.disease.db$class, k = num.folds) #"FIX ME" DONE
...
for(i in 1:length(cost.parameter)){
  ...
  for(j in 1:num.folds){
    ...
    # *** ATTENTION !!! *** #
    svm.model <- svm(formula = class ~ ., data = train.set, kernel = "radial", cost = cost.parameter[i], probability = TRUE) #"FIX ME" DONE
...
# [YOUR ANSWER HERE]
# Answer: 
# The cost parameter to choose is the one with the highest average accuracy, i.e. the 
# cost parameter that resulted in the best prediction. I would in this case choose the 
# cost parameter to be 1.0 (which means cost = 1.0). This resulted in an avg.accuracy = 0.8352874
...
# *** ATTENTION !!! *** #
svm.pred <- predict(svm.model, newdata = test.set, probability = TRUE) #"FIX ME" DONE

class.probs <- attr(svm.pred, "probabilities")
pred2 <- prediction(class.probs[,2], test.set$class)

# *** ATTENTION !!! *** #
roc.curve <- performance(pred2, "tpr", "fpr") #"FIX ME" DONE
